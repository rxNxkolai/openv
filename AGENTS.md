...
**The store records what a session was measured against, not only what it
measured.** Each session persists its zone set (name, kind, polygon) and whether
pose ran. Three defects shared the absence of that: a shelf with no reaches left
no row, so the deadest fixture was the one `analyze` could not mention; `measure`
matched sessions on a zone's name alone and produced a confident p-value across a
moved polygon; and a `.db` was unreadable without the zones file beside it. Now a
drawn shelf watched with pose on and never reached is a `dead` finding, `measure`
returns `not_comparable` when the polygon changed, and the live console opens a
fresh session when its zones change so one session id never mixes two boundaries.
A session that predates this reports its zones as unrecorded, not as empty.
